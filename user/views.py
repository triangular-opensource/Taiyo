...
class BidChanges(generics.ListAPIView, generics.UpdateAPIView, generics.DestroyAPIView):
    serializer_class = NotificationSerializer
    def get(self, request, *args, **kwargs):
        serializer = self.get_serializer(Notification.objects.filter(advertisement=kwargs['id']),many=True)
        return success_response(serializer.data)

    def put(self, request, *args, **kwargs):
        try:
            bid = Notification.objects.get(id=kwargs['id'])
    ...
    def delete(self, request, *args, **kwargs):
        instance = generics.get_object_or_404(Notification,   advertisement =kwargs['id'])
        instance.delete()
